        ...
        for(sptIndex mode=0; mode<nmodes; ++mode) {
            ...
            if(dev_id == -2) {
                ...
                if(par_iters == 0) {
                    ...
                    sptAssert(sptOmpMTTKRPHiCOO_MatrixTiling_Scheduled(&hitsr, U, mats_order, mode, nt, 0) == 0);
                } else {
                    printf("sptOmpMTTKRPHiCOO_MatrixTiling_Scheduled_Reduce:\n");
                    sptAssert(sptOmpMTTKRPHiCOO_MatrixTiling_Scheduled_Reduce(&hitsr, U, copy_U, mats_order, mode, nt, 0) == 0);
                }
            }
